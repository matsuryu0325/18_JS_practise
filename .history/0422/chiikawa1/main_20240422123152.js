...
for (let i = 0; i < Chiikawas.length; i++) {
    Chiikawa.push(new Chiikawa(Chiikawas[i].story, Chiikawas[i].title, Chiikawas[i].image, Chiikawas[i].id));
    //メソッドの呼び出し
    //console.log(chiikawaContents[i].createMarkup());//dl...dt...dd
    container.insertAdjacentHTML("beforeend", Chiikawa[i].createMarkup());
}
